...
from utils_drl import AgentP
from utils_env import MyEnv
from utils_memory import PrioritizedReplayMemory
...
memory = PrioritizedReplayMemory(STACK_SIZE + 1, MEM_SIZE, device)
...
for step in progressive:
    ...
    delta = agent.calc_delta(
        state,
        torch.Tensor([[action]]).long().to(device),
        torch.cat([state.squeeze()[1:, :, :],obs.float().to(device)]).unsqueeze(0).float().to(device),
        torch.Tensor([[done]]).float().to(device),
        torch.Tensor([[reward]]).float().to(device),
        ).item()

    memory.push(env.make_folded_state(obs_queue), action, reward, done, delta)

    if step % POLICY_UPDATE == 0 and training:
        loss = agent.learn(memory, BATCH_SIZE)
        loss_file.write(f"{step//POLICY_UPDATE:3d} {step:8d} {loss:.6f}\n")
        # if win is None:
